...
> **Created:** 2026-02-25 | **Updated:** 2026-03-01
...
- For test results and known issues, see [e2e-test-results.md](./e2e-test-results.md).
...
```
Buyer requests part (phone call-in or website) →
GOPA forwards request to seller(s) →
Sellers bid → Buyer accepts bid (web or USSD) → Cart →
Checkout / MoMo payment (web or USSD) →
Seller prepares → Rider picks up → Rider delivers
```
...
| 2 | **Seller Bids (USSD)** | Seller mocker → `*887*9#` → 1. Open Requests → 1. All Requests → select request → enter price + delivery date → confirm with PIN. | Bid created (status: Pending). Buyer gets SMS about new bids. |
| 3 | **Buyer Accepts Bid (USSD)** | Buyer mocker → `*887*9#` → 2. My requests → select request → view bids → accept bid. | Bid added to cart. Seller bid status → Accepted. |
| 4 | **Buyer Checkout (USSD)** | Buyer mocker → 3. Cart → 1. Check Out → select delivery option → MoMo payment (CalBank sandbox). | Order confirmation shows order code + delivery code. Invoice created. |
| 5 | **GOPA Forwards Request to Seller** | GOPA dials `*887*9#` → Browse requests → selects request → forwards to specific seller. | Request forwarded. Seller can see it under Open Requests. |
| 5b | **GOPA Tracks Forwarded Order** | GOPA dials `*887*9#` → My forwarded orders → views status of orders they forwarded. | Order statuses visible (bid received, accepted, shipped, delivered). |
| 6 | **Seller Marks Ready (USSD)** | Seller mocker → `*887*9#` → 2. My bids → select accepted bid → mark "Ready for Pickup" → PIN. | Status → READY_TO_BE_PICKED. Visible in rider's New pickups. |
| 7 | **Rider Accepts Pickup (USSD)** | Rider mocker → `*887*9#` → 1. New pickups → select pickup → confirm. | Rider assigned. Item moves to In-progress deliveries. |
| 8 | **Rider Picks Up** | Rider mocker → 2. In progress deliveries → select delivery → confirm pickup from seller. | Status → SHIPPED. Item remains in In-progress deliveries. |
| 9 | **Rider Delivers (Delivery Code)** | Rider mocker → 2. In progress deliveries → select delivery → enter delivery code (buyer provides verbally). | Status → DELIVERED. Delivery moves to 3. Completed deliveries. |
| 10 | **Rider Wallet (USSD)** | Rider mocker → `*887*9#` → 4. Wallet → verify total balance, available to withdraw, lifetime earnings → 1. Withdraw → enter 4-digit PIN. | Balance correct. Withdrawal processed. |
| 11 | **Wallet Settlement** | Check seller wallet (pending credit, 24hr hold). Check rider wallet via USSD (option 4). Verify amounts match order. | All wallets credited correctly. |
...
| 15 | **GOPA Onboards Seller (USSD)** | GOPA dials `*887*9#` → Onboard seller → enters seller details (name, phone, location). | Seller account created and linked to GOPA. Seller receives SMS confirmation. *(May not be implemented in USSD yet — document current state.)* |
| 15b | **Seller Browses by Part/Brand (USSD)** | Seller mocker → `*887*9#` → 1. Open Requests → 2. By part (or 3. By brand) → browse filtered requests. | Requests filtered correctly by part name or brand. |
...
| 21 | **GOPA Forwards Request to Seller** | Log in as GOPA → Unassigned Requests → forward request to specific seller(s). Check Forwarded Requests. | Request forwarded. Seller can see and bid on it. |
...
| 25 | **GOPA Tracks Forwarded Orders** | Log in as GOPA → Forwarded Orders dashboard → verify statuses of orders they forwarded (bid received, accepted, shipped, delivered). | Order statuses accurate. GOPA can see which seller fulfilled. |
...
| 31 | **GOPA Onboards Seller (Web)** | Log in as GOPA → Onboard Seller → enter seller details (name, phone, store, location, payment account). | Seller account created and linked to GOPA. Seller can log in. |
...
**Total: 48 tasks** (6 admin + 12 USSD + 8 USSD edge cases + 12 web + 5 web edge cases + 5 cross-cutting)
