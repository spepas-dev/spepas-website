...
const ActiveBidsPage: React.FC = () => {
  ...
  if (!sellerId) {
    return <div className="text-center text-red-600 mt-12 px-4">Seller ID missing in URL</div>;
  }
        ...
        <div className="flex gap-4 border-b border-gray-200">
          <button
            ...
            className={`pb-1 border-b-2 transition ${
              tab === 'new' ? 'border-indigo-600 text-indigo-600 font-medium' : 'border-transparent text-gray-600'
            }`}
          >
            New Requests
          </button>
          <button
            ...
            className={`pb-1 border-b-2 transition ${
              tab === 'history' ? 'border-indigo-600 text-indigo-600 font-medium' : 'border-transparent text-gray-600'
            }`}
          >
            Submitted Bids
          </button>
        ...
        <>
          ...
          <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 mb-6">
            <span className="font-semibold text-base">
              {loading ? 'Loading requests…' : `${bids.length} request${bids.length !== 1 ? 's' : ''}`}
            </span>
