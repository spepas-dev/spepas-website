...
const OffersList: React.FC = () => {
  ...
  if (!requestId) {
    return <p className="text-center text-red-500 mt-10 px-4">No request selected.</p>;
  }
  if (loading) {
    return (
  ...
  if (!offers.length) {
    return <p className="text-center text-gray-500 mt-10 px-4">No offers yet.</p>;
  }
  ...
  return (
    <div className="px-4 sm:px-6 md:px-8 lg:px-0 space-y-8">
      ...
      <div className="bg-indigo-50 border-l-4 border-indigo-500 rounded-lg shadow mx-auto max-w-3xl p-6">
        <h2 className="text-xl sm:text-2xl font-semibold text-indigo-700 mb-4">Request Details</h2>
        <div className="flex flex-col sm:flex-row items-center sm:items-start">
          {img && <img src={img} alt={sparePart.name} className="w-full h-48 sm:w-24 sm:h-24 object-cover rounded mb-4 sm:mb-0 sm:mr-6" />}
          <div className="flex-1 space-y-2 text-center sm:text-left">
            <h3 className="text-lg sm:text-xl font-bold text-gray-800">{sparePart.name}</h3>
            <p className="text-sm sm:text-base text-gray-600">{sparePart.description}</p>
            <p className="text-sm">
              Quantity requested: <strong className="text-gray-800">{quantity}</strong>
            </p>
      ...
      >
        {offers.map((o) => (
          <div key={o.bidding_ID} className="w-full max-w-sm">
            <OfferCard offer={o} inCart={!!cartMap[o.bidding_ID]} onAdd={handleAdd} onRemove={handleRemove} />
          </div>
        ))}
      </div>
    </div>
