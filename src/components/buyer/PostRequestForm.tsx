...
import { getCarBrands, getCarManufacturers, getCarModels } from '@/lib/inventoryApis';
  ...
  useEffect(() => {
    getCarManufacturers()
      .then((res) => {
        ...
        if (prefill?.partName) {
          setName(prefill.partName);
        }
        if (prefill?.manufacturerName) {
          const mfr = mfrs.find((m) => m.name.toLowerCase() === prefill.manufacturerName!.toLowerCase());
          if (mfr) {
  ...
  useEffect(() => {
    ...
    if (mfr?.brands?.length) {
      ...
      if (prefill?.brandName) {
        const brand = mfr.brands.find((b) => b.name.toLowerCase() === prefill.brandName!.toLowerCase());
        if (brand) {
          setSelectedBrand(brand.CarBrand_ID);
        }
      }
      return;
    }
    ...
    getCarBrands({ manufacturerId: selectedManufacturer })
      .then((res) => {
        ...
        setBrands(fetched);
        if (prefill?.brandName) {
          const brand = fetched.find((b) => b.name.toLowerCase() === prefill.brandName!.toLowerCase());
          if (brand) {
            setSelectedBrand(brand.CarBrand_ID);
          }
        }
