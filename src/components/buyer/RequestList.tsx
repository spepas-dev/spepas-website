...
const RequestList: React.FC<RequestListProps> = ({ mode }) => {
  ...
  const filteredData = data.filter((req) => {
    ...
    if (endDate && created > new Date(`${endDate}T23:59:59`)) {
      return false;
    }
    return true;
  });
  ...
  if (!data.length) {
    return (
      <div className="bg-white rounded-2xl shadow-sm border border-gray-100 p-12 text-center space-y-3">
        ...
        <p className="text-sm text-gray-500">No {mode === 'active' ? 'active' : 'historical'} requests found.</p>
      </div>
    );
  }
        ...
        <div className="flex flex-col sm:flex-row items-center gap-3">
          ...
          <button
            ...
            className={`inline-flex items-center gap-2 text-sm font-medium px-4 py-2.5 rounded-xl border transition ${
              showFilters ? 'bg-blue/10 text-blue border-blue/20' : 'bg-white text-gray-600 border-gray-200 hover:bg-gray-50'
            }`}
          >
              ...
              {Array.from({ length: totalPages }, (_, i) => i + 1).map((num) => (
                <button
                  ...
                  className={`w-8 h-8 rounded-lg text-sm font-medium transition ${
                    num === currentPage ? 'bg-blue text-white' : 'text-gray-600 hover:bg-gray-100'
                  }`}
                >
                  {num}
                </button>
