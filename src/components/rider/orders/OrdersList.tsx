...
const Badge: React.FC<{ children: React.ReactNode; tone?: 'info' | 'success' }> = ({ children, tone = 'info' }) => (
  <span className={`text-xs px-2 py-1 rounded-full ${tone === 'success' ? 'bg-violet-100 text-violet-700' : 'bg-amber-100 text-amber-700'}`}>{children}</span>
);
...
const OrderMiniCard: React.FC<{ o: OrderRow; onClick: () => void }> = ({ o, onClick }) => (
  ...
  >
    ...
    <div className="mt-2">
      <Badge tone={o.status === 'delivered' ? 'success' : 'info'}>
...
const OrdersList: React.FC<{ initialTab?: 'in_progress' | 'delivered' }> = ({ initialTab = 'in_progress' }) => {
  ...
  const allOrders: OrderRow[] = useMemo(() => ([
    { id: '32516',  date: '14/06/2023', price: 50, status: 'in_progress', pickup: 'Ghana Spare Parts Ltd., Otu Adzin Road, GT-444, Ghana', dropoff: 'Otu Adzin Road, GT-444, Ghana' },
    { id: '32517A', date: '14/06/2023', price: 70, status: 'delivered',   pickup: 'Circle Station, Accra', dropoff: 'Dansoman Roundabout, Accra' },
      ...
      <div className="space-y-3">
        {filtered.map(o => (
          <OrderMiniCard
            key={o.id}
            o={o}
            onClick={() => navigate(`/95668339501103956045/rider/orders/${o.id}`)}
          />
        ))}
        {filtered.length === 0 && (
          <div className="text-sm text-gray-500 border rounded-md p-4 text-center">No orders.</div>
        )}
      </div>
    </div>
  );
