...
const ShopWithoutSidebar: React.FC = () => {
  ...
  // ══════════════════════════════════════════════════════════════════════════
  // Query — car model variants (for fuel/body/drive/engine filter values)
  // ══════════════════════════════════════════════════════════════════════════
  ...
  const fuelTypeOptions = useMemo(() => {
    // Show fuels available given body/drive/engine selection (but not fuel itself)
  ...
  const vehicleSummary = useMemo(() => {
    ...
    const mfr = manufacturersData?.data?.find((m: any) => m.Manufacturer_ID === selectedMake);
    const mdl = brandsForMake.find((b: any) => b.CarBrand_ID === selectedModel);
    const parts = [mfr?.name, mdl?.name].filter(Boolean);
    if (selectedYear) parts.unshift(selectedYear);
    return parts.join(' ');
  }, [selectedMake, selectedModel, selectedYear, manufacturersData, brandsForMake]);
            ...
            <div className={`grid grid-cols-1 gap-3 ${hasYears ? 'md:grid-cols-3' : 'md:grid-cols-2'}`}>
              ...
              <div>
                ...
                <SearchableCombobox
                  ...
                  placeholderLabel={selectedMake ? 'e.g. Corolla, 3 Series, Tucson' : 'Select manufacturer first'}
                ...
                <div className="flex flex-wrap items-center gap-x-4 gap-y-2.5">
                  ...
                  {fuelTypeOptions.length > 0 && (
                    <div className="flex items-center gap-1.5">
                      ...
                      {fuelTypeOptions.map((ft: string) => (
                        <button
                          key={ft}
                          onClick={() => updateParams({ fuel: selectedFuelType === ft ? '' : ft, page: '' })}
                  ...
                  {bodyTypeOptions.length > 0 && (
                    <div className="flex items-center gap-1.5">
                      ...
                      {bodyTypeOptions.map((bt: string) => (
                        <button
                          key={bt}
                          onClick={() => updateParams({ body: selectedBodyType === bt ? '' : bt, page: '' })}
                          className={`px-2.5 py-1 rounded-full text-xs font-medium transition-all ${
                            selectedBodyType === bt
                              ? 'bg-[var(--color-primary-500)] text-white shadow-sm'
                              : 'bg-gray-100 text-gray-600 hover:bg-gray-200'
                  ...
                  {driveTypeOptions.length > 0 && (
                    <div className="flex items-center gap-1.5">
                      ...
                      {driveTypeOptions.map((dt: string) => (
                        <button
                          key={dt}
                          onClick={() => updateParams({ drive: selectedDriveType === dt ? '' : dt, page: '' })}
                          className={`px-2.5 py-1 rounded-full text-xs font-medium transition-all ${
              ...
              {!vehicleSelected && (
                <div className="flex flex-col items-center justify-center py-20 text-center">
                  ...
                  <p className="text-sm text-gray-400 max-w-xs">
                    Choose a {hasYears ? 'year, manufacturer, and model' : 'manufacturer and model'} above to browse available spare parts.
                  </p>
                </div>
              )}
