...
const Testimonials: React.FC = () => {
  ...
  const prev = useCallback(() => {
    setIndex((i) => (i === 0 ? testimonialsData.length - visible : Math.max(0, i - 1)));
  }, [visible]);

  const next = useCallback(() => {
    setIndex((i) => (i + visible >= testimonialsData.length ? 0 : i + 1));
  }, [visible]);
      ...
      <div className="max-w-4xl mx-auto px-4">
        ...
        <div className="flex items-center justify-between mb-8">
          ...
          <div className="flex gap-4">
            <button onClick={prev} className="p-2 bg-gray-200 rounded-full hover:bg-gray-300 transition">
              ‹
            </button>
            <button onClick={next} className="p-2 bg-gray-200 rounded-full hover:bg-gray-300 transition">
              ›
            </button>
          </div>
        </div>
